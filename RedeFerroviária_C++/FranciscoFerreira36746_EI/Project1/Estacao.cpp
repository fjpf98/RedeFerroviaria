...
bool Estacao::guardaPassagem(Passagem* p)
{
	return estPassagens.insert(*p);
}
...
{
	Colecao<Passagem>::iterator itEstacao;
	for (itEstacao = estPassagens.begin(); itEstacao != estPassagens.end(); itEstacao++)
	{
		itEstacao->printEsta();
		if (itEstacao->getcomParagem() == true)
		{
			cout << "Parou! " << endl;
		}
		else
		{
			cout << "Passou sem parar! " << endl;
		}
	}
